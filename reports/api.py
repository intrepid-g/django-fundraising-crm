...
# Report Endpoints
@api.get("/reports", response=List[ReportSchema])
def list_reports(
    request,
...
@api.get("/reports/{report_id}", response=ReportSchema)
def get_report(request, report_id: int):
    """Get a specific report configuration."""
    return Report.objects.get(id=report_id)


@api.post("/reports", response=ReportSchema)
...
@api.put("/reports/{report_id}", response=ReportSchema)
...
@api.post("/reports/{report_id}/execute")
def execute_report(
    request,
    report_id: int,
    date_range_start: Optional[date] = None,
...
@api.get("/reports/{report_id}/executions", response=List[ReportExecutionSchema])
...
# Dashboard Endpoints
@api.get("/dashboards", response=List[DashboardSchema])
...
@api.get("/dashboards/{dashboard_id}", response=DashboardSchema)
def get_dashboard(request, dashboard_id: int):
    """Get a specific dashboard."""
    return Dashboard.objects.get(id=dashboard_id)


@api.post("/dashboards", response=DashboardSchema)
...
@api.put("/dashboards/{dashboard_id}", response=DashboardSchema)
...
@api.get("/dashboards/{dashboard_id}/data")
...
# Metric Endpoints
@api.get("/reports/metrics", response=List[MetricDefinitionSchema])
...
@api.get("/reports/metrics/{metric_slug}")
